...
class StateOP(NamedTuple):
    ...
    end_ids: torch.Tensor  # Ending index
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        graph_size = n_loc + 2 if is_depot2 else n_loc + 1
        coords = torch.cat((depot[:, None, :], loc, depot2[:, None, :]), -2) if is_depot2 else torch.cat((depot[:, None,
                                                                                                          :], loc), -2)
        return StateOP(
            coords=coords,
            ...
            max_length=max_length[:, None] - (depot2[:, None, :] - coords).norm(p=2, dim=-1) - 1e-6 if is_depot2 else
                       max_length[:, None] - (depot[:, None, :] - coords).norm(p=2, dim=-1) - 1e-6,
            ...
            end_ids=graph_size - 1 if is_depot2 else 0
        )

    def get_remaining_length(self):
        # max_length[:, 0] is max length arriving at depot so original max_length
        return self.max_length[self.ids, self.end_ids] - self.lengths
    ...
    def all_finished(self):
        ...
        return self.i.item() > 0 and (self.prev_a == self.end_ids).all()
